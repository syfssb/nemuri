...
**Free와 Pro는 이 저장소에 있는 같은 코드를 실행합니다.** 루트 헬퍼, `pmset`/센티널/배터리 프리미티브, 설정 인스톨러, hook 브리지 — 두 등급이 하나를 함께 쓰며, 따로 만든 버전은 없습니다. Pro를 사든 사지 않든, 당신의 신뢰가 필요한 부분은 여기 공개된 바로 이 부분입니다.
...
macOS 13+와 Swift 5.9+가 필요합니다. 이 트리는 단독으로 빌드됩니다 — 비공개 소스 의존성도, 서드파티 패키지도, 빌드 시 네트워크 접근도 없습니다.
...
Nemuri는 서명·공증된 DMG로 배포하며, 이 저장소의 [Releases](https://github.com/syfssb/nemuri/releases) 페이지에 올라갑니다. **v1.0은 아직 나오지 않았습니다** — 이 저장소에서 Watch → Releases를 눌러 두면 나올 때 GitHub이 알려줍니다. 제품에 대한 자세한 설명은 <https://nemuri.app/ko/>를 참고하세요.

### Free — 수동 keep-awake 스위치

![Nemuri Free 패널: Agent Mode 켜짐, 세션 목록 없음, agent 감지는 Pro 기능으로 표시](docs/panel-free.png)

켜면 덮개를 닫아도 Mac이 깨어 있습니다. 외장 디스플레이도 필요 없습니다. 무료 앱이 하는 일은 여기까지이고, 감지도 자동화도 없습니다. 당신이 직접 끌 때까지 계속 깨워 두며, 세션 목록은 언제까지나 비어 있습니다 — 패널은 있는 척하지 않고 그렇다고 알려줍니다.

### Pro — 감지 자동화

![Nemuri Pro 패널: agent 2개 감시 중, 하나는 확인 대기, 하나는 실행 중, 끝나면 슬립 복원](docs/panel-pro.png)

Pro는 **에이전트가 실제로 일하는 동안에만** Mac을 깨워 둡니다. 그래서 Agent Mode는 켜 둔 채로 잊어버려도 됩니다. 어떤 에이전트가 실행 중이고 어떤 에이전트가 당신의 확인을 기다리는지 알려주고, 마지막 하나가 끝나면 슬립을 되돌려 놓습니다. $19 1회 결제이고, 라이선스 하나로 보유한 모든 Mac에서 쓸 수 있습니다. **이 패널 뒤의 감지 엔진은 클로즈드 소스이며 이 저장소에 없습니다.**

**두 등급 모두:** 절대 걸리지 않기 위한 4가지 복구 경로, 배터리 보호, 로그인 시 자동 실행, 그리고 같은 업데이트를 받습니다. 업데이트가 수동인 것은 의도한 것입니다. 타이머를 걸어 대신 확인해 주는 프로그램은 결국 스스로 네트워크에 나가는 프로그램이고, 그것이야말로 이 저장소가 배제하려는 것이기 때문입니다. 설정 → 정보에서 「업데이트 확인…」을 누르는 그때, 그때만 Nemuri가 온라인에 접속합니다. Free도 같고, 같은 경로를 씁니다.
